...
//     var my_api_key = '74e9324ddf26ffda8ad02ea8bf8e6853';
//     var title = $('.movie').val();
//     if (title=="") {
...
$.ajax({
  ...
  success: function (data) {
    ...
    topCast.forEach((member) => {
      ...
      cast_chars.push(member.character);
      cast_profiles.push(`https://image.tmdb.org/t/p/original${member.profile_path}`);
    });
  },
});
return cast;
...
$(function() {
  // Tugmani faollashtirish yoki deaktivatsiya qilish
  const source = document.getElementById('autoComplete');
  const inputHandler = function(e) {
    if(e.target.value == ""){
      $('.movie-button').attr('disabled', true);
  ...
  // Movie qidiruvi tugmasi
  $('.movie-button').on('click', function() {
    var my_api_key = 'YOUR_API_KEY'; // TMDb API kaliti
    var title = $('.movie').val();
    if (title == "") {
      $('.results').css('display', 'none');
      $('.fail').css('display', 'block');
    } else {
      console.log(`Qidirilayotgan movie: ${title}`);
      load_details(my_api_key, title);
    }
  });
});

// Tavsiyalar bo‘yicha karta bosilganda
function recommendcard(e) {
  var my_api_key = 'YOUR_API_KEY';
  var title = e.getAttribute('title');
  console.log(`Tavsiyalar uchun tanlangan movie: ${title}`);
  load_details(my_api_key, title);
}

// Movie tafsilotlarini olish
function load_details(my_api_key, title) {
  $.ajax({
    type: 'GET',
    url: `https://api.themoviedb.org/3/search/movie?api_key=${my_api_key}&query=${title}`,
    success: function(movie) {
      if (movie.results.length < 1) {
        console.log('Movie topilmadi.');
        $('.fail').css('display', 'block');
        $('.results').css('display', 'none');
      } else {
        var movie_id = movie.results[0].id;
        var movie_title = movie.results[0].original_title;
        console.log(`Movie topildi: ${movie_title}, ID: ${movie_id}`);
        movie_recs(movie_title, movie_id, my_api_key);
      }
    },
    error: function(xhr, status, error) {
      console.error('API chaqiruvi xatosi:', error);
    }
  });
}

// Tavsiyalarni backenddan olish
function movie_recs(movie_title, movie_id, my_api_key) {
  $.ajax({
    type: 'POST',
    url: "/similarity",
    ...
    success: function(recs) {
      if (recs.includes("Sorry")) {
        console.log('Tavsiyalar mavjud emas.');
        $('.fail').css('display', 'block');
        $('.results').css('display', 'none');
      } else {
        console.log('Tavsiyalar:', recs);
        var movie_arr = recs.split('---');
...
// Movie tafsilotlarini olish va ko‘rsatish
function get_movie_details(movie_id, my_api_key, arr, movie_title) {
  $.ajax({
    type: 'GET',
    url: `https://api.themoviedb.org/3/movie/${movie_id}?api_key=${my_api_key}`,
    success: function(movie_details) {
      console.log('Movie tafsilotlari:', movie_details);
      show_details(movie_details, arr, movie_title, my_api_key, movie_id);
    },
    error: function(xhr, status, error) {
      console.error('Movie tafsilotlarini olishda xato:', error);
    }
  });
}

// Movie tafsilotlarini ko‘rsatish
function show_details(movie_details, arr, movie_title, my_api_key, movie_id) {
  var poster = `https://image.tmdb.org/t/p/original${movie_details.poster_path}`;
  var overview = movie_details.overview;
  var genres = movie_details.genres.map(genre => genre.name).join(", ");
  console.log('Movie haqida:', { poster, overview, genres });
  ...
  $.ajax({
    ...
    error: function(xhr, status, error) {
      console.error('Tafsilotlarni yuborishda xato:', error);
    }
  });
}
