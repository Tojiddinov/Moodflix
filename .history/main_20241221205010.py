...
def rcmd(movie):
    ...
    if movie not in data['movie_title'].str.lower().unique():
        ...
        idx = data.loc[data['movie_title'].str.lower() == movie].index[0]
        scores = list(enumerate(similarity[idx]))
        scores = sorted(scores, key=lambda x: x[1], reverse=True)
        scores = scores[1:11]
        recommended = [data['movie_title'][i[0]] for i in scores]
...
#         poster = request.form.get('poster', '/static/default_poster.png')
...
#         return render_template('recommend.html', title=title, poster=poster, genres=genres, 
#                                overview=overview, runtime=runtime, release_date=release_date, 
